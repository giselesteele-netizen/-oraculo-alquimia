...
/* ══════════════════════════════════════════════════════
   MENSAGENS PRÉ-ESCRITAS POR ARCANJO E TEMA
══════════════════════════════════════════════════════ */
const MENSAGENS = {
  Miguel: [
    `Amada alma guerreira,\n\nSinto em você uma batalha silenciosa — algo que você tem carregado sozinha há tempo demais. Mas saiba: a armadura de luz que Miguel oferece não é para quem não precisa de proteção, é para quem tem coragem de continuar mesmo quando o caminho dói. Você tem essa coragem. Eu a vejo.\n\nAgora é hora de erguer sua espada interior. Diga não ao que drena sua energia. Corte os vínculos que te prendem no passado. Você não precisa de permissão de ninguém para escolher sua paz. Proteja seus sonhos com a mesma ferocidade com que protege as pessoas que ama.\n\nEU SOU LUZ, EU SOU PROTEÇÃO, EU SOU LIVRE. A FORÇA DE MIGUEL CAMINHA COMIGO E NADA ME ABALA.`,
    `Guerreiro(a) de luz,\n\nVocê chegou até aqui. Isso já é vitória. Mas percebo que algo ainda tenta te fazer duvidar do seu valor — uma voz antiga que não é sua, mas que você aprendeu a acreditar. Hoje Miguel te convida a questionar essa voz. Ela mente.\n\nA verdade é que você foi feito(a) para mais. Cada obstáculo no seu caminho foi um treinamento, não uma punição. Levante a cabeça. O que parece uma derrota agora é o solo onde seu próximo capítulo vai germinar. Confie.\n\nEU REJEITO O QUE NÃO ME PERTENCE. EU AVANÇO COM FÉ E CORAGEM. MIGUEL ILUMINA CADA PASSO DO MEU CAMINHO.`,
  ],
  Gabriel: [
    `Alma sensível e iluminada,\n\nGabriel vem até você com uma mensagem urgente da sua própria intuição: você já sabe a resposta que está buscando. Sabe. Mas o medo de estar errado(a), de decepcionar, de perder — isso tem te impedido de agir. Hoje é o dia de parar de esperar um sinal maior. Você É o sinal.\n\nConfie no que seu coração sussurra nas madrugadas. Aquele sonho que não te deixa em paz não é ilusão — é chamado. Aquela ideia que volta sempre não é loucura — é propósito. O universo conspira a seu favor quando você tem coragem de acreditar primeiro.\n\nEU OUÇO A VOZ DA MINHA ALMA. EU CONFIO NA MINHA INTUIÇÃO. GABRIEL ILUMINA MEU CAMINHO COM CLAREZA E FÉ.`,
    `Querida alma,\n\nHá uma mensagem que você precisa entregar — seja ao mundo, a alguém específico ou a si mesmo(a). Gabriel é o arcanjo das revelações, e ele vem te dizer: é hora de falar. De se expressar. De parar de engolir o que sente.\n\nSua voz importa. Sua história tem poder. O que você viveu — inclusive a dor — é combustível para transformar outras almas. Não subestime o impacto da sua presença genuína no mundo. Quando você aparece como realmente é, você dá permissão para outros fazerem o mesmo.\n\nEU ME EXPRESSO COM VERDADE E AMOR. MINHA VOZ É UM INSTRUMENTO DE LUZ. EU TENHO O QUE O MUNDO PRECISA OUVIR.`,
  ],
  Rafael: [
    `Alma abençoada,\n\nRafael chega até você com a energia da cura — não porque você está quebrado(a), mas porque você merece florescer inteiro(a). Há um cansaço em você que vai além do físico. É a alma que pediu uma pausa. É o seu ser que grita por cuidado.\n\nPermita-se receber. Você que tanto cuida dos outros — quando foi a última vez que cuidou de você com a mesma dedicação? Hoje, o arcanjo da cura te pede um gesto de amor próprio. Pode ser pequeno. O que importa é a intenção.\n\nEU MEREÇO SER CURADO(A) E AMADO(A). EU ABRO MEU CORAÇÃO PARA RECEBER. RAFAEL RESTAURA CADA CÉLULA DO MEU SER COM LUZ VERDE E AMOR.`,
    `Ser de luz,\n\nA abundância que você busca já está em movimento. Rafael cuida não só da cura do corpo e da alma — ele cuida dos recursos que sustentam sua missão aqui. E sua missão é real, é importante, é necessária.\n\nDesbloqueie a crença de que não merece prosperidade. Saúde, dinheiro, amor, paz — tudo isso é manifestação da mesma energia: a energia do SIM que o universo quer te dar. Abra as mãos. Abra o coração. O que você precisa está a caminho.\n\nEU SOU UM CANAL DE ABUNDÂNCIA E CURA. O UNIVERSO PROVÊ TUDO QUE PRECISO. EU RECEBO COM GRATIDÃO E GRAÇA.`,
  ],
  Uriel: [
    `Alma em busca de clareza,\n\nUriel, a chama da sabedoria, ilumina o que estava nas sombras: a resposta que você procura está dentro de você, não fora. Você tem buscado validação, aprovação, sinais externos — mas a sabedoria verdadeira só nasce do silêncio interno.\n\nPare. Respire. O que você sabe, no fundo, que precisa mudar? Que decisão você tem adiado por medo de errar? Uriel te diz: errar em direção à sua verdade é sempre melhor do que acertar vivendo a vida de outro. A chama que te guia é interior. Confie nela.\n\nEU CONFIO NA SABEDORIA DA MINHA ALMA. EU TOMO DECISÕES ALINHADAS COM MINHA LUZ. URIEL ILUMINA CADA ESCOLHA DO MEU CAMINHO.`,
    `Querida alma,\n\nA confusão que sente agora não é sinal de que você está perdido(a) — é sinal de que está crescendo. Quando a visão antiga não serve mais e a nova ainda não está clara, existe esse intervalo desconfortável. Você está nele. E é sagrado.\n\nUriel te convida a não fugir desse espaço. Fique. Observe. O que está morrendo em você para que algo novo nasça? Honre o processo. A clareza vem para quem tem paciência de ficar no escuro sem entrar em pânico.\n\nEU ABRAÇO MINHA TRANSFORMAÇÃO COM CORAGEM. A LUZ DE URIEL DISSOLVE TODA CONFUSÃO. EU EMERJO MAIS SÁBIO(A) E PLENO(A).`,
  ],
  Metatron: [
    `Alma em ascensão,\n\nMetatron, guardião dos Registros Akáshicos, te vê além desta vida. O que você carrega agora — os padrões que se repetem, as dores que não têm explicação lógica — tem raízes profundas. Mas saiba: você veio nessa encarnação especificamente para transmutar isso.\n\nVocê não é vítima da sua história — você é o alquimista dela. Cada experiência difícil foi um contrato de alma aceito por um ser poderoso o suficiente para transformá-la em luz. Você é esse ser. Agora é hora de lembrar quem você é.\n\nEU TRANSMUTO TODA DOR EM SABEDORIA E LUZ. EU ACESSO O PODER DOS MEUS REGISTROS AKÁSHICOS. EU SOU UMA ALMA EM PLENA ASCENSÃO.`,
    `Ser multidimensional,\n\nVocê está em um momento de virada de chave — um portal de transformação que poucos reconhecem quando estão dentro dele. Metatron te diz: não é por acaso que você está aqui, lendo isso, nesse momento específico.\n\nSua alma escolheu acordar. E despertar dói às vezes — porque significa enxergar o que estava adormecido, soltar o que era confortável, aceitar uma versão sua que ainda não se conhece bem. Mas do outro lado desse portal está a versão de você que sempre soube que existia.\n\nEU ACEITO MEU DESPERTAR ESPIRITUAL. EU SOLTO O QUE NÃO ME SERVE MAIS. METATRON GUIA MINHA ALMA PARA SUA MAIS ALTA EXPRESSÃO.`,
  ],
  Chamuel: [
    `Alma amada,\n\nChamuel chega com o amor mais puro — não o amor romântico que tanto buscamos fora, mas o amor que cura tudo: o amor próprio. E ele te pergunta com gentileza: como você tem se tratado ultimamente? Com que palavras você fala consigo mesmo(a) quando erra?\n\nO amor que você deseja receber começa no espelho. Não como clichê — como prática diária de escolher ser gentil com você mesmo(a). De se perdoar. De se ver com os mesmos olhos com que vê quem você ama. Você merece esse amor. Sem condição.\n\nEU ME AMO E ME ACEITO COMPLETAMENTE. MEU CORAÇÃO ESTÁ ABERTO PARA DAR E RECEBER AMOR. CHAMUEL ENVOLVE MEU SER COM A LUZ DO AMOR INCONDICIONAL.`,
    `Coração aberto,\n\nHá uma cura de coração que está acontecendo em você — mesmo que você não perceba ainda. Antigas mágoas, decepções, amores que doeram. Chamuel vem para dizer: você não precisa mais carregar isso. Perdoar não é dizer que foi certo — é se libertar do peso.\n\nE sobre o amor que você busca: ele começa quando você para de se diminuir para caber na vida de alguém. Você não precisa se fazer menor. A pessoa, o relacionamento, a conexão certa não vai te pedir para ser menos do que você é.\n\nEU LIBERTO MEU CORAÇÃO DE TODA MÁGOA E RESSENTIMENTO. EU SOU DIGNO(A) DO AMOR QUE DESEJO. CHAMUEL CURA E ABRE MEU CORAÇÃO PARA O AMOR VERDADEIRO.`,
  ],
};
  ...
  return (
    <svg viewBox="0 0 200 280" style={{ width:"100%", height:"100%" }}>
      ...
      {[[14,14],[186,14],[14,266],[186,266]].map(([cx,cy],i)=>(
        <g key={i} transform={`translate(${cx},${cy})`}><circle r="3" fill={a.acento} opacity="0.5"/></g>
      ))}
      <g transform="translate(100,118)" filter={`url(#gw${a.id})`}>
        ...
        {a.id===1 && <>
          ...
          {[[-38,-28],[38,-18],[48,18],[-48,8],[0,-52],[0,52]].map(([sx,sy],i)=>(
            <g key={i} transform={`translate(${sx},${sy})`}>
              <polygon points="0,-4 1.2,1.2 -1.2,1.2" fill={a.brilho} opacity={0.4+i*0.07}/>
              <polygon points="0,4 1.2,-1.2 -1.2,-1.2" fill={a.brilho} opacity={0.4+i*0.07}/>
            </g>
          ))}
        </>}
...
/* ══════════════════════════════════════════════════════
   TYPEWRITER
══════════════════════════════════════════════════════ */
function Typewriter({ texto, velocidade = 14 }) {
  const [exibido, setExibido] = useState("");
  const ref = useRef(null);
  const prevTexto = useRef("");
...
function Estrelas() {
  const pts = useRef(Array.from({ length: 80 }, () => ({
    x: Math.random()*100, y: Math.random()*100,
    r: Math.random()*1.3+0.2, o: Math.random()*0.3+0.04,
...
function Label({ children }) { return <p style={{ fontFamily:"'Cinzel',serif",fontSize:9,letterSpacing:3,color:"rgba(232,200,122,.38)",marginBottom:9,textTransform:"uppercase" }}>{children}</p>; }
function Btn({ children, onClick, disabled, style }) {
  return <button onClick={onClick} disabled={disabled} style={{ width:"100%",padding:"16px",background:"linear-gradient(135deg,rgba(232,200,122,.1),rgba(200,150,10,.07))",border:"1px solid rgba(232,200,122,.32)",borderRadius:2,color:"#e8c87a",fontFamily:"'Cinzel',serif",fontSize:11,letterSpacing:3,cursor:disabled?"not-allowed":"pointer",transition:"all .3s",opacity:disabled?.4:1,textTransform:"uppercase",...style }}>{children}</button>;
}
function BtnSm({ children, onClick }) {
  return <div style={{ display:"flex",justifyContent:"center" }}><button onClick={onClick} style={{ padding:"9px 24px",background:"transparent",border:"1px solid rgba(232,200,122,.18)",borderRadius:2,color:"rgba(232,200,122,.55)",fontFamily:"'Cinzel',serif",fontSize:10,letterSpacing:2,cursor:"pointer" }}>{children}</button></div>;
}
function Cabecalho({ titulo, emoji, onVoltar }) {
  return (
...
function CTA() {
  return (
    <div style={{ marginTop:24,padding:"18px",textAlign:"center",border:"1px solid rgba(232,200,122,.1)",borderRadius:4,background:"rgba(232,200,122,.025)" }}>
      <p style={{ fontSize:13,fontStyle:"italic",color:"rgba(232,200,122,.48)",marginBottom:12,lineHeight:1.7 }}>Esta é uma prévia. Para uma leitura completa dos seus Registros Akáshicos, entre em contato.</p>
      <button onClick={()=>window.open("https://instagram.com/giselesteele.oficial","_blank")} style={{ padding:"12px 24px",background:"linear-gradient(135deg,rgba(232,200,122,.1),rgba(200,150,10,.07))",border:"1px solid rgba(232,200,122,.28)",borderRadius:2,color:"#e8c87a",fontFamily:"'Cinzel',serif",fontSize:9,letterSpacing:3,cursor:"pointer",textTransform:"uppercase" }}>→ Agendar Leitura Completa</button>
  ...
  return (
    <div style={{ animation:"aparecer .7s ease" }}>
      <div style={{ textAlign:"center",marginBottom:32 }}>
        ...
        <h1 style={{ fontFamily:"'Cinzel',serif",fontSize:28,fontWeight:700,letterSpacing:2,lineHeight:1.15,background:"linear-gradient(120deg,#c8a030,#f0d880,#e8c060,#c8a030)",backgroundSize:"300% auto",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",backgroundClip:"text",animation:"shimmer 5s linear infinite" }}>Portal Espiritual</h1>
      ...
      <div style={{ marginTop:28,padding:"16px",textAlign:"center",borderTop:"1px solid rgba(232,200,122,.07)" }}>
        ...
        <a href="https://instagram.com/giselesteele.oficial" target="_blank" rel="noreferrer" style={{ fontFamily:"'Cinzel',serif",fontSize:9,letterSpacing:4,color:"rgba(232,200,122,.4)",textDecoration:"none" }}>@giselesteele.oficial</a>
      </div>
    </div>
  );
}
...
function TelaOraculo({ onVoltar }) {
  ...
  function consultar() {
    ...
    setVirada(false);
    setEtapa("virando");
    setTimeout(() => setVirada(true), 300);
    setTimeout(() => {
      setMensagem(getMensagem(a.nome));
      setEtapa("resultado");
    }, 1200);
  }
  function reiniciar() { setEtapa("form"); setTema(""); setArcIdx(null); setMensagem(""); setVirada(false); }
  const a = arcIdx !== null ? ARCANJOS[arcIdx] : null;

  return (
    <div style={{ animation:"aparecer .5s ease" }}>
      <Cabecalho titulo="Oráculo Angelical" emoji="🔮" onVoltar={onVoltar}/>
      {etapa==="form" && (
        <div>
          <Label>Tema</Label>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:22 }}>
            {ARCANJOS.map((ar,i)=>(
              <button key={ar.id} onClick={()=>setArcIdx(i)} style={{ padding:"11px 6px",borderRadius:4,cursor:"pointer",transition:"all .25s",textAlign:"center",
                ...
                <div style={{fontFamily:"'Cinzel',serif",fontSize:11,color:arcIdx===i?ar.brilho:"rgba(232,200,122,.6)"}}>{ar.nome}</div>
              </button>
            ))}
          </div>
      ...
      {etapa==="virando" && a && (
        <div style={{ textAlign:"center",padding:"32px 0" }}>
          <div style={{ display:"flex",justifyContent:"center",marginBottom:20 }}>
            <CartaFlip arcanjo={a} virada={virada} tamanho={130} hoverable={false}/>
          </div>
          <p style={{ fontFamily:"'Cinzel',serif",fontSize:10,letterSpacing:4,color:"rgba(232,200,122,.42)",animation:"breathe 2s ease-in-out infinite" }}>REVELANDO...</p>
        </div>
      )}
      {etapa==="resultado" && a && (
...
function TelaBaralho({ onVoltar }) {
  ...
  function escolher(i) {
    ...
    setTimeout(() => {
      setViradas({[i]:true});
      setTimeout(() => {
        setMensagem(getMensagem(ARCANJOS[ordem[i]].nome));
        setEtapa("resultado");
      }, 800);
    }, 300);
  }
  function reiniciar() { setEtapa("form"); setIntencao(""); setOrdem([]); setEscolhida(null); setViradas({}); setMensagem(""); }
  const a = escolhida !== null && ordem.length ? ARCANJOS[ordem[escolhida]] : null;

  return (
    <div style={{ animation:"aparecer .5s ease" }}>
      <Cabecalho titulo="Baralho das Cartas" emoji="🃏" onVoltar={onVoltar}/>
      {etapa==="form" && (
        <div>
      ...
      {etapa==="embaralhando" && (
        <div style={{ textAlign:"center",padding:"28px 0" }}>
          <div style={{ display:"flex",justifyContent:"center",position:"relative",height:90,marginBottom:18 }}>
            {[0,1,2,3,4,5].map(i=>(
              <div key={i} style={{ width:50,height:70,borderRadius:5,overflow:"hidden",position:"absolute",left:`calc(50% + ${(i-2.5)*14}px)`,top:0,animation:`${i%2===0?"shuffleL":"shuffleR"} .5s ${i*.08}s ease-in-out infinite`,boxShadow:"0 4px 14px rgba(0,0,0,.6)" }}><CardBack/></div>
            ))}
          </div>
...
function TelaSpread({ onVoltar }) {
  ...
  function virar(i) {
    ...
    if (nv.every(Boolean) && !mensagem) {
      ...
      const msg = `${getMensagem(past.nome)}\n\n— — —\n\n${getMensagem(pres.nome)}\n\n— — —\n\n${getMensagem(fut.nome)}`;
      setTimeout(() => { setMensagem(msg); setEtapa("resultado"); }, 600);
    }
  }
  function reiniciar() { setEtapa("form"); setIntencao(""); setCartas([]); setViradas([false,false,false]); setMensagem(""); }

  return (
    <div style={{ animation:"aparecer .5s ease" }}>
      <Cabecalho titulo="Tiragem 3 Cartas" emoji="✨" onVoltar={onVoltar}/>
      {etapa==="form" && (
        <div>
      ...
      {(etapa==="spread"||etapa==="resultado") && cartas.length===3 && (
        <div>
          {etapa==="spread" && !viradas.every(Boolean) && (
            <p style={{ textAlign:"center",fontStyle:"italic",color:"rgba(232,200,122,.4)",fontSize:14,marginBottom:18,lineHeight:1.6 }}>Toque em cada carta para revelar</p>
          )}
          ...
          {etapa==="resultado" && mensagem && (
            <div style={{ animation:"aparecer .6s ease" }}>
              <MsgBox mensagem={mensagem}/>
              <CTA/><BtnSm onClick={reiniciar}>✦ Nova Tiragem</BtnSm>
            </div>
          )}
        </div>
      )}
